...
namespace BehaviorTreeFrame
{
    ...
    ///以提供实时性和性能。
    /// </summary>
    class SelectorParallelNode<Entity> : CompositeNode<Entity>
    {


        public SelectorParallelNode():base()
        {

        }
        public SelectorParallelNode(ConditionNode<Entity> condition) : base(condition)
        {
        ...
        {
            BTResultStatus result = BTResultStatus.Ended;//默认为TRUE
            for(int i = 0; i < ChildNodes.Count; i++)//不管怎么样都会执行所有的子节点
            {
                if (ChildNodes[i].DoTick() == BTResultStatus.Error)
                {
                    result = BTResultStatus.Error;//设定返回值，一FALSE都FALSE
                }
            }
